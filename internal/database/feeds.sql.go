...
func (q *Queries) GetFeedList(ctx context.Context) ([]GetFeedListRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.FeedName, &i.Url, &i.CreatorName); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
UPDATE feeds SET updated_at = NOW(), last_fetched_at = NOW() WHERE id = $1 RETURNING id, created_at, updated_at, name, url, user_id, last_fetched_at
`
